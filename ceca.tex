...
\subsection{D'où vient l'idée}
L'idée d'E-Jam est directement venue de nos besoins respectifs. En effet, un grand nombre
de personnes constituant notre équipe pratiquent un instrument et ont déjà œuvré au sein
...
volume de chaque instrument, réglages des pédales d'effets...) nous ont poussé à nous poser
...
de pouvoir créer une plate-forme de répétition en ligne dont chaque membre d'un groupe
puisse se connecter et répéter en live tout en restant chez soi, c'est-à-dire : avec son matériel
et son confort de vie. En outre, une telle application permet au musiciens de réaliser de
nombreuses économies sur le long terme. \\
En effet :
\begin{itemize}
\item Le coût de la salle de répétition n'est plus un frais pour aucun membre du groupe.
\item Les coûts de transports (voiture, transports en commun, train) sont bannis.
D'autres part, des économies en temps sont réalisées :
\item Les temps de transports sont inexistants.
\item Le temps d'installation commun au groupe est largement minoré puisque chaque
musicien dispose du confort d'installation de son équipement chez lui. (Le batteur par
exemple n'a pas à régler la tension de ses peaux, installer ses propres cymbales et modifier
...
\begin{itemize}
    ...
    E-Jam propose aussi un système d’événements pour permettre au gens de se socialiser. Un événement indique la création d’une session à une date donnée ainsi que les caractéristiques de cette session. Un musicien peut alors créer un événement ou marquer sa participation. L’utilisateur peut consulter son agenda avec la liste des évènements auxquels il participe, des évènements proposé par ses amis ainsi que des évènements qui peuvent l'intéresser.\\
    ...
    \\
    Une que le musicien est dans un session il peut jouer de la musique et entendre les autres membres de la session. Il a aussi accès à des outils pour facilité l’organisation de la session:
    \begin{itemize}
        \item accordeur
...
E-Jam propose aussi une plate-forme en ligne pour les spectateurs. Les spectateurs peuvent créer un compte mais ça n’est pas obligé. Un peu à la manière des plate-forme de vidéo en ligne l’utilisateur se verra proposé des sessions sur la page principale et pourra aussi rechercher avec des mots-clés et des noms de musiciens ou groupes.\\
...
Selon un ingénieur du son le temps de délais maximal imperceptible était de l’ordre de 30ms. Aussi nous avons fait différents tests:
\begin{itemize}
    ...
    Pour savoir à partir de quel délai on pouvait percevoir la différence entre deux son en condition réelle:\\
    On jouait sur un clavier virtuelle qui jouait la note et l’envoyait via internet mais sur le même ordinateur. Notre ordinateur réceptionnait ensuite le paquet internet et rejouait la note.\\
    On règle simule ensuite des temps de latence réelle sur le réseau local via netem un outils du noyau linux qui ajoute un délai à l'envoi de paquet sur internet.\\
    \\
...
\textbf{Guitar Land Grenoble :}
    ...
    \begin{itemize}
        \item l'ajout d'un accordeur en ligne simple et intuitif permettant aux musiciens de définir un accordage commun et en relation avec le style de musique joué durant la session de répétition. (accordage spéciaux pour le blues, le jazz, le classique).
        ...
        \item Enfin, notre contact a déclaré qu'il aurait été souhaitable d'intégrer un module vidéo de type skype permettant aux musiciens de dialoguer entre eux (avant le début d'une session par exemple) et ainsi se mettre d'accord sur la suite de notes, de rythme, de tonalité avec lesquelles ils répéteront. Cette fonctionnalité, très utile permettrait aux musiciens d'apprécier pleinement les répétitions en fixant un certain cadre pour tous et ainsi d'éviter toute frustration du à des problèmes de compréhension par exemple.
    \end{itemize}
Notre contact a également déclaré qu'il donnait quelques cours de musiques. Nous lui avons présenté E-Jam comme une plate-forme en ligne lui permettant de donner des cours en ligne de chez lui. L'idée lui semblait bonne et selon lui, pourrait découler sur une autre facette d'E-Jam. Il a toutefois déclaré qu'il n'utiliserait pas l'application dans ce sens puisque personnellement, il lui est important de corriger ses élèves jusque dans leur gestes (position des doigts sur un manche de guitare, posture générale…). A l'inverse, nous avons consulté de nombreuses personnes disant qu'elles révéraient d'obtenir des cours en ligne de musiciens célèbres et n'hésiteraient nullement à payer ces cours plus cher !\\
\\
...
Elle a évoquer le fait que les sessions devait permettre de s’organiser facilement avec notamment un tchat, la gamme utilisé, la grille d’accord, l’ordre des improvisations, leurs durées, etc.\\
Le logiciel lui semblait utile pour pouvoir répéter avec des amis sans toujours se déplacer mais aussi de pouvoir jouer beaucoup plus facilement avec des gens. En effet on passe beaucoup de temps seul chez soi avec son instrument donc permettre aux de jouer à plusieurs sans sortir de chez eux serait intéressant pour pas mal de musiciens.
...
Finalement cet entretiens nous a permis de réfléchir à de nouvelle fonctionnalité pour l’interface utilisateurs des sessions. Et aussi d’aborder plus en profondeur les fonctionnalité sociale qu’on pourrait proposer.
...
D’autres sites comme jammr ou sofasession par exemple, proposent également de jouer en simultanée, mais souffrent d’une communauté réduite (moins de 1000 likes sur facebook), ou d’un site en beta et donc non-fonctionnel pour l’instant. Le problème de la latence est souvent résolu par la méthode que ninjam (une mesure de retard). Nous avons noté que Sofasession dispose d’une interface très facile et agréable d’utilisation, et d’un aspect réseau social bien pensé. 
...
\end{itemize}
Enfin toujours pour développer la communauté nous ajouterons au logiciel l'interface pour les spectateurs, nous organiserons des évènements avec des guests et nous permettrons à des groupes de se faire connaître notamment avec l’aide de tremplins.\\
\\
...
Un hébergeur (comme Youtube) n'est pas résponsable des contenus illicites. Il n'est pas soumis à une obligation générale de surveillance. Sa responsabilité n'est pas engagé, s'ils n'ont pas eu connaissance effective de la présence du contenu illicite. Il est responsable uniquement s'il n'agit pas pour retirer le contenu litigieux. Il n'a pas à retirer le contenu illicite sans notification préalable et même si ce contenu a déjà fait l'objet d'un signalement et a déjà été retiré une première fois.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{SWOT.png}
